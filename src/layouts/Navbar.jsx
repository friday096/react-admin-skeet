...
const Navbar = () => {
  const { setAuthToken } = useAuth(); // Assuming this is where you're managing the auth token
  const [_, __, removeLocalToken] = useLocalStorage('authToken', null); // Get remove
  const navigate = useNavigate();
  const handleLogout = async () => {
    removeLocalToken('authToken'); // Remove the auth token
   
    setAuthToken(null); // Also clear it from the Auth context if needed
    navigate('/auth/login');
